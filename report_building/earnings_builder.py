...
class EarningsBuilder(ReportBuilder):
    ...
    def build_markdown(self, earnings_data: list[EarningsInformation]) -> str:
        ...
        for earning in earnings_data:
            ...
            md.append(
                f"- **{self.localization.translate("earnings_date")}** {earning.date.strftime('%d.%m.%Y')}"
            )
            if earning.eps is not None:
                md.append(
                    f"- **{self.localization.translate("earnings_eps_avg")}** {earning.eps.avg}"
                )
                md.append(
                    f"- **{self.localization.translate("earnings_eps_low")}** {earning.eps.low}"
                )
                md.append(
                    f"- **{self.localization.translate("earnings_eps_high")}** {earning.eps.high}"
                )
            else:
                md.append(
                    f"- **{self.localization.translate("earnings_eps_avg")}** N/A"
                )
                md.append(
                    f"- **{self.localization.translate("earnings_eps_low")}** N/A"
                )
                md.append(
                    f"- **{self.localization.translate("earnings_eps_high")}** N/A"
                )

            md.append(f"\n**{self.localization.translate("earnings_prev_eps")}**\n")
            md.append(
                f"| | {self.localization.translate("earnings_latest")} | -> | -> | {self.localization.translate("earnings_oldest")} |"
            )
            md.append("|---|---|---|---|---|")
            helper_row = (
                f"| **{self.localization.translate("earnings_prev_eps_expected")}** |"
            )
            ...
            helper_row = (
                f"| **{self.localization.translate("earnings_prev_eps_actual")}** |"
            )
            for prev in earning.previous_earnings:
            ...
            helper_row = (
                f"| **{self.localization.translate("earnings_prev_eps_diff")}** |"
            )
            for prev in earning.previous_earnings:
            ...
            md.append(
                f"- **{self.localization.translate("earnings_market_cap")}** {market_cap_str}"
            )
            md.append(
                f"- **{self.localization.translate("earnings_revenue")}** {revenue_str}"
            )

            if graph_path:
                md.append(
                    f"\n**{self.localization.translate("earnings_price_chart")}**\n"
                )
