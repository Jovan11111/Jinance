...
from providers.yahoo.yahoo_price_performance_provider import YahooPricePerformanceProvider
...
class TestPricePerformanceManager:
    ...
    def test__initialize_pp_manager_negative_days_ahead__fields_initialize_with_default_value(self):
    ...
    def test__initialize_pp_manager_with_default_values__initialized_with_default_values(self):
    ...
    def test__initialize_pp_manager_non_existant_provider__initialized_with_default_provider(self):
        """Check if PP manager is initialized with default provider, if a fake provider is given."""
        pp_manager = PricePerformanceManager("NON_EXISTANT", 456, constants.TICKERS_SP_10)

        assert pp_manager.days_ahead == 456
        assert pp_manager.tickers == constants.TICKERS_SP_10
        assert isinstance(pp_manager.provider, YahooPricePerformanceProvider)

    def test__get_price_performance_good_data__return_list_of_winners_and_losers(self, mock_yahoo_price_performance_provider):
    ...
    def test__get_price_performance_empty_list__returns_empty_list(self, mock_yahoo_price_performance_provider):
    ...
    def test__get_price_performance_negative_number_of_companies__sets_number_of_companies_to_3(self, mock_yahoo_price_performance_provider):
    ...
    def test__get_price_performance_small_data__returns_lesser_winners_losers(self, mock_yahoo_price_performance_provider, create_price_performance_info):
        """Check if get_worst_best_price_performance returns same for winners and losers when there are less tickers then companies."""
        mock_yahoo_price_performance_provider.fetch_price_performance.return_value = create_price_performance_info[:2]
        pp_manager = PricePerformanceManager()
        result = pp_manager.get_best_worst_price_performance(3)
